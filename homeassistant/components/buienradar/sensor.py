...
def _load_data(self, data):  # noqa: C901
    ...
    if self._is_duplicate_measurement(data):
        return False
    ...
    if sensor_type == SYMBOL or sensor_type.startswith(CONDITION):
        return self._handle_condition_sensor(sensor_type, data)

    if sensor_type.startswith(PRECIPITATION_FORECAST):
        return self._handle_precipitation_forecast(sensor_type, data)

    if sensor_type in [WINDSPEED, WINDGUST]:
        return self._handle_wind_sensor(sensor_type, data)

    if sensor_type == VISIBILITY:
        return self._handle_visibility_sensor(sensor_type, data)

    return self._handle_default_sensor(sensor_type, data)

    def _is_duplicate_measurement(self, data):
        """Return True if this data has already been measured."""
        return self._measured == data.get(MEASURED)
    
    
    def _get_forecast_day(self, sensor_type):
        """Map suffix to forecast day offset."""
        suffix_map = {"_1d": 0, "_2d": 1, "_3d": 2, "_4d": 3, "_5d": 4}
        for suffix, day in suffix_map.items():
            if sensor_type.endswith(suffix):
                return day
        return 0
    
    
    def _handle_forecast_sensor(self, sensor_type, data):
        fcday = self._get_forecast_day(sensor_type)
    
        if sensor_type.startswith((SYMBOL, CONDITION)):
            return self._handle_forecast_condition(sensor_type, data, fcday)
    
        if sensor_type.startswith(WINDSPEED):
            return self._handle_forecast_wind(sensor_type, data, fcday)
    
        return self._handle_forecast_default(sensor_type, data, fcday)
    
    
    def _handle_forecast_condition(self, sensor_type, data, fcday):
        try:
        ...
        new_state = self._extract_condition_state(sensor_type, condition)
    ...
    def _extract_condition_state(self, sensor_type, condition):
        """Return appropriate condition state depending on sensor type."""
    ...
    def _handle_forecast_wind(self, sensor_type, data, fcday):
        try:
    ...
    def _handle_forecast_default(self, sensor_type, data, fcday):
        try:
            self._attr_native_value = data.get(FORECAST)[fcday].get(sensor_type[:-3])
        except IndexError:
            _LOGGER.warning("No forecast for fcday=%s", fcday)
    ...
    def _handle_condition_sensor(self, sensor_type, data):
        if not (condition := data.get(CONDITION)):
            return False
    ...
    def _handle_precipitation_forecast(self, sensor_type, data):
    ...
    def _handle_wind_sensor(self, sensor_type, data):
    ...
    def _handle_visibility_sensor(self, sensor_type, data):
        self._attr_native_value = data.get(sensor_type)
        if self.state is not None:
    ...
    def _handle_default_sensor(self, sensor_type, data):
        self._attr_native_value = data.get(sensor_type)
